...
export type ComplaintAttachment = {
  ...
  uploadedBy: { staffId: string; fullName: string | null } | null
  createdAt: string
}
...
/** GET /complaints/:ref and /complainant/complaints/:ref — the list omits the timeline. */
export type PublicComplaintDetail = PublicComplaint & {
  timeline: StatusTimelineEntry[]
}

/** GET /auth/setup — true only while no staff account exists (local only). */
...
export type StaffAccount = {
  ...
  isActive: boolean
  hasPassword: boolean
  /** Blocked after 5 failed passwords until ADMIN unlocks (§8 decision 14). */
  locked: boolean
...
/** GET/PUT /admin/settings/security (ADMIN). */
